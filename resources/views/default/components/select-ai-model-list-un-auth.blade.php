@php
    ...
    // Initialize OpenRouter models if registered
    $openRouterModels = [];
    if (MarketplaceHelper::isRegistered('open-router')) {
        try {
            $openRouterModels = OpenRouterEngine::cases();
        } catch (\Exception $e) {
            \Log::warning('Failed to load OpenRouter models: ' . $e->getMessage());
            $openRouterModels = [];
        }
    }

    // Merge all models and ensure uniqueness
    $fullModels = collect()
        ->when($defaultModel, fn($collection) => $collection->push($defaultModel))
    ...
    if ($defaultModel?->value) {
        try {
            $entityEnum = EntityEnum::tryFrom($defaultModel->value);
            $selectedModel = $entityEnum ? EntityFacade::driver($entityEnum) : null;
        } catch (\Exception $e) {
            \Log::warning('Failed to initialize selected model driver: ' . $e->getMessage());
            $selectedModel = null;
        }
    }

    // Fallback if no selected model
    if (!$selectedModel && $fullModels->isNotEmpty()) {
        try {
            $firstModel = EntityEnum::tryFrom($fullModels->first()?->value);
            $selectedModel = $firstModel ? EntityFacade::driver($firstModel) : null;
        } catch (\Exception $e) {
            $selectedModel = null;
        }
    }
    ...
    // Pre-load model drivers to avoid N+1 queries in the loop
    $modelDrivers = collect();
    foreach ($fullModels as $model) {
        try {
            $entityEnum = EntityEnum::fromSlug($model?->value);
            if ($entityEnum) {
                $driver = EntityFacade::driver($entityEnum);
                $modelDrivers->put($model->value, $driver);
            }
        } catch (\Exception $e) {
            \Log::warning("Failed to load driver for model {$model?->value}: " . $e->getMessage());
        }
    }
@endphp
...
<div x-data="modelList">
    ...
    >
        ...
        <x-slot:head-content>
            <div class="flex flex-wrap items-center justify-between gap-y-3 sm:grow lg:flex-nowrap">
                <div class="grow">
                    <h4 class="mb-0">
                        @lang('AI Models')
                    </h4>
                ...
                >
                    <div class="flex flex-col items-end">
                            ...
                            @forelse ($fullModels as $engine)
                                ...
                                <x-card
                                    ...
                                    @class([
                                        ...
                                        'data-[selected]:outline data-[selected]:outline-[3px] data-[selected]:outline-secondary',
                                        'opacity-60 cursor-not-allowed' => !$isAccessible,
                                    ])
                                    variant="outline-shadow"
                                    @click.prevent="updateSelectedModels({
                                        value: $el.getAttribute('data-model-value'),
                                        label: $el.getAttribute('data-model-label')
                                    })"
                                    ::data-selected="isModelSelected($el.getAttribute('data-model-value'))"
                        ...
                        <x-button
                            ...
                            type="submit"
                            size="xl"
                            ::disabled="!selectedModels.length || selectedModels[0].value === ''"
                        >
                            {{ __('Apply') }}
                        </x-button>
                    </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('modelList', () => ({
                ...
                toggleCouncilMode() {
                    if (this.councilMode && !this.isAuthenticated) {
                ...
                updateSelectedModels(model) {
                    ...
                    this.$refs.modelsSelectElement.value = model.value;
                    this.$refs.modelsSelectElement.dispatchEvent(new Event('change'));
                },

                showLoginToast() {
                    if (typeof toastr !== 'undefined') {
                        toastr.warning('{{ __('Please login to access premium AI models') }}', '{{ __('Login Required') }}');
                    } else {
                        alert('{{ __('Please login to access premium AI models') }}');
                    }
                },

                isModelSelected(modelValue) {
                    return this.selectedModels.findIndex(model => model.value === modelValue) >= 0;
                }
            }));
        });
    </script>
