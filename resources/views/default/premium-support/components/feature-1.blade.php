@php
	$features = cache()->remember('extensions_list', 120, function () {
		$extensionService = app(\App\Domains\Marketplace\Repositories\Contracts\ExtensionRepositoryInterface::class);
		$extensionsArray = $extensionService->paidExtensions();
		$themesArray = $extensionService->themes();
		$extensionNames = collect($extensionsArray)->pluck('name')->toArray();
		$themeNames = collect($themesArray)->pluck('name')->toArray();
		return array_merge($extensionNames, $themeNames);
	});
@endphp
