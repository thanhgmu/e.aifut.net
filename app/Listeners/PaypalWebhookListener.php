    ...
    public function handle(PaypalWebhookEvent $event): void
    {
        try {
            ...
            if ($event_type === 'PAYMENT.SALE.COMPLETED') {
                $newData->parent_payment = null;
                if (isset($incomingJson->resource) && is_object($incomingJson->resource)) {
                    $newData->parent_payment = $incomingJson->resource->parent_payment ?? null;
                }
            ...
            if ($event_type === 'BILLING.SUBSCRIPTION.CANCELLED') {
                ...
                if (array_key_exists('error', $transactions) === false) {
                    ...
                    Log::error('PayPal Transaction Response: ' . json_encode($transactions));
                }
            }
