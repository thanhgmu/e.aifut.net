...
class AIRealTimeChatController extends Controller
{
    protected $settings;
    ...
    /**
     * Get API key for WebSocket connection for Flutter app
     *
     * @OA\Get(
     *      path="/api/airealtimechat/websocket-credentials",
     *      operationId="getWebsocketCredentials",
     *      tags={"Realtime Chat"},
     *      summary="Get API key for WebSocket connection for Real Time Chat",
     *      description="Returns the API key parts needed to establish a WebSocket connection",
     *      security={{ "passport": {} }},
     *
     *      @OA\Response(
     *          response=200,
     ...
     *              type="object",
     *
     *              @OA\Property(property="apikeyPart1", type="string"),
     *              @OA\Property(property="apikeyPart2", type="string"),
     *              @OA\Property(property="apikeyPart3", type="string"),
    ...
    public function getWebsocketCredentials(Request $request)
    {
        ...
        return response()->json([
            ...
            'key2'  => $apikeyPart2,
            'key3'  => $apikeyPart3,
            'model' => 'gpt-4o-realtime-preview-2024-12-17', // Default model used in web frontend
        ], 200);
    }

    /**
     * Save a response from the mobile app
     *
     * @OA\Post(
     *      path="/api/airealtimechat/save-conversation",
     *      operationId="saveConversation",
     *      tags={"Realtime Chat"},
     *      summary="Save a voice chat conversation from the mobile app",
     *      description="Saves a voice chat interaction from the mobile app to the database",
     *      security={{ "passport": {} }},
     *
     *      @OA\RequestBody(
     *          required=true,
     *          description="Conversation data to save",
     *
     *          @OA\JsonContent(
     *              type="object",
     *
     *              @OA\Property(property="userMessage", type="string", description="User's spoken message"),
     *              @OA\Property(property="aiResponse", type="string", description="AI's response"),
     *              @OA\Property(property="chatId", type="integer", description="Existing Chat ID (optional)"),
     *          ),
     *      ),
     *
     *      @OA\Response(
    ...
    public function saveConversation(Request $request)
    {
        try {
            ...
            if (! $request->has('userMessage') && empty($request->userMessage)) {
                return response()->json(['status' => 'error', 'message' => 'User message is required', 'chatId' => 0], 412);
            }

            if (! $request->has('aiResponse') && empty($request->aiResponse)) {
                return response()->json(['status' => 'error', 'message' => 'AI response is required', 'chatId' => 0], 412);
            }

            if ($request->has('chatId') && ! empty($request->chatId)) {
                $chat = UserOpenaiChat::where('id', $request->chatId)
            ...
            if (! $chat) {
                ...
                $category = OpenaiGeneratorChatCategory::where('slug', 'ai_realtime_voice_chat')->first();
                if (! $category) {
                    // Fallback to first category if ai_realtime_voice_chat doesn't exist
                    $category = OpenaiGeneratorChatCategory::first();
                }
            ...
            $used_tokens = countWords($request->aiResponse);

            $aiMessage = new UserOpenaiChatMessage;
            $aiMessage->user_openai_chat_id = $chat->id;
            $aiMessage->user_id = $request->user()->id;
            $aiMessage->input = $request->userMessage;
            $aiMessage->response = $request->aiResponse;
            $aiMessage->output = $request->aiResponse;
            $aiMessage->hash = Str::random(256);
            $aiMessage->credits = $used_tokens;
            $aiMessage->words = 0;
            $aiMessage->save();

            $driver->input($request->aiResponse)->calculateCredit()->decreaseCredit();
            $chat->total_credits += $used_tokens;
            $chat->save();
            ...
            Log::error('Error saving conversation from mobile app: ' . $e->getMessage());
